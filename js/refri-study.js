// Refrigeracion Comercial e Industrial Study System — Maestro HVACR
...
(function() {
  ...
  function _getByCategory(cat) {
    if (!window.REFRI_QUESTIONS) return [];
    if (cat === 'ALL') return window.REFRI_QUESTIONS.slice();
    return window.REFRI_QUESTIONS.filter(function(q) { return q.categoria === cat; });
  }
  function _loadQuizProgress(cat, quizNum) {
    try { return JSON.parse(localStorage.getItem('refri_quiz_' + cat + '_' + quizNum)) || null; }
    catch(e) { return null; }
  }
  function _saveQuizProgress(cat, quizNum, correctas, total) {
    try { localStorage.setItem('refri_quiz_' + cat + '_' + quizNum, JSON.stringify({ correctas: correctas, total: total, fecha: new Date().toISOString() })); }
    catch(e) {}
  }
  function _loadSimulacroHistory() {
    try { return JSON.parse(localStorage.getItem('refri_simulacros')) || []; }
    catch(e) { return []; }
  }
  function _saveSimulacroResult(result) {
    try {
      ...
      localStorage.setItem('refri_simulacros', JSON.stringify(hist));
    } catch(e) {}
  }

  var CAT_KEYS = ['REFRI_DOMESTICA', 'REFRI_COMERCIAL', 'REFRI_INDUSTRIAL', 'REFRI_CONTROLES', 'REFRI_DIAGNOSTICO'];
  var THEME_PRIMARY = '#00838F';
  var THEME_DARK = '#006064';

  // ── Main Init ──
  window.initRefriStudy = function() {
    var screen = document.getElementById('refriStudyScreen');
    if (!screen) return;
    _activeTab = 'estudiar';
    _renderMain(screen);
  };
  ...
  function _renderMain(screen) {
    var totalQ = window.REFRI_QUESTIONS ? window.REFRI_QUESTIONS.length : 0;
    var html = '';
    ...
    html += '<div style="flex:1;text-align:center;"><h2 style="color:#fff;margin:0;font-size:18px;">Refrigeraci\u00f3n Comercial e Industrial</h2>';
    ...
    html += '<div style="display:flex;gap:4px;background:rgba(0,0,0,0.2);border-radius:10px;padding:3px;" id="refriTabs">';
    var tabs = [
    ...
    tabs.forEach(function(tab) {
      ...
      html += '<button onclick="_refriSwitchTab(\'' + tab.id + '\')" style="flex:1;padding:8px 4px;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;' +
    ...
    html += '</div></div>';
    html += '<div id="refriContent" style="padding:12px;overflow-y:auto;flex:1;"></div>';
    screen.innerHTML = html;
    _renderTabContent();
  }

  window._refriSwitchTab = async function(tabId) {
    ...
    _activeTab = tabId;
    var tabsDiv = document.getElementById('refriTabs');
    if (tabsDiv) {
  ...
  function _renderTabContent() {
    var container = document.getElementById('refriContent');
    if (!container) return;
  ...
  function _renderEstudiar(container) {
    var cats = window.REFRI_CATEGORIAS || {};
    ...
    CAT_KEYS.forEach(function(catKey) {
      ...
      html += '<div class="refri-cat-section" id="refriCat_' + catKey + '">';
      html += '<div onclick="document.getElementById(\'refriCat_' + catKey + '\').classList.toggle(\'refri-cat-open\')" style="background:linear-gradient(135deg,' + catInfo.color + ',' + catInfo.color + 'cc);padding:12px 14px;border-radius:12px;cursor:pointer;display:flex;align-items:center;gap:10px;margin-bottom:2px;transition:all 0.2s;">';
      ...
      html += '<span style="color:#fff;font-size:18px;transition:transform 0.3s;" class="refri-cat-arrow">\u25bc</span>';
      html += '</div>';

      html += '<div class="refri-cat-body" style="display:none;padding:8px 0;">';
  ...
  function _injectStyles() {
    if (document.getElementById('refriStudyStyles')) return;
    var style = document.createElement('style');
    style.id = 'refriStudyStyles';
    style.textContent =
      '.refri-cat-section .refri-cat-body{display:none}' +
      '.refri-cat-section.refri-cat-open .refri-cat-body{display:block!important}' +
      '.refri-cat-section.refri-cat-open .refri-cat-arrow{transform:rotate(180deg)}' +
      '.refri-cat-section{margin-bottom:8px}';
  ...
  function _renderPracticar(container) {
    ...
    CAT_KEYS.forEach(function(catKey) {
      ...
      html += '<div onclick="_refriShowQuizzes(\'' + catKey + '\')" style="background:#fff;border:2px solid ' + catInfo.color + '33;border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;transition:all 0.2s;">';
    ...
    html += '<div onclick="_refriStartPractice(\'ALL\',0)" style="background:linear-gradient(135deg,#667eea,#764ba2);border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;">';
  ...
  window._refriShowQuizzes = function(cat) {
  ...
  function _renderQuizSelector(container) {
    ...
    html += '<button onclick="_refriBackToCategories()" style="background:none;border:none;color:#3D3D3A;cursor:pointer;font-size:13px;padding:0 0 8px;">\u2190 ' + (typeof _t === 'function' ? _t('study_back_categories', 'Volver a Categorias') : 'Volver a Categorias') + '</button>';
    ...
    for (var qi = 1; qi <= numQuizzes; qi++) {
      ...
      html += '<div onclick="_refriStartPractice(\'' + cat + '\',' + qi + ')" style="background:#fff;border:2px solid ' + (completed ? (passed ? '#86efac' : '#fca5a5') : catInfo.color + '33') + ';border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;">';
  ...
  window._refriBackToCategories = function() {
    _practiceView = 'categories';
    _practiceSelectedCat = null;
    _renderTabContent();
  };

  window._refriStartPractice = function(cat, quizNum) {
    var questions = _getByCategory(cat);
  ...
  function _renderPracticeQuestion(container) {
    ...
    var catInfo = (window.REFRI_CATEGORIAS || {})[q.categoria] || { nombre: q.categoria, color: '#666' };
    var total = st.questions.length;
    var num = st.currentIndex + 1;
    var letters = ['A', 'B', 'C', 'D'];

    var html = '';
    html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;padding:8px 12px;background:#f0fdfa;border-radius:10px;">';
    html += '<button onclick="_refriExitPractice()" style="background:none;border:none;color:#3D3D3A;cursor:pointer;font-size:13px;">\u2190 Salir</button>';
    ...
    _qf(q,'opciones').forEach(function(opt, oi) {
      ...
      html += '<button onclick="_refriPracticeAnswer(' + oi + ')" style="' + btnStyle + '">';
    ...
    if (st.answered) {
      ...
      html += '</div>';
      html += '<button onclick="_refriPracticeNext()" style="display:block;width:100%;margin-top:14px;padding:14px;background:linear-gradient(135deg,' + THEME_PRIMARY + ',' + THEME_DARK + ');color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;">';
  ...
  window._refriPracticeAnswer = function(idx) {
    ...
    if (idx === q.respuesta_correcta) {
      ...
      if (window.Gamification) window.Gamification.recordAnswer('refri', q.categoria || q.category || '', true);
    } else {
      _practiceState.incorrectas++;
      _practiceState.streak = 0;
      if (window.Gamification) window.Gamification.recordAnswer('refri', q.categoria || q.category || '', false);
    }
    _renderPracticeQuestion(document.getElementById('refriContent'));
  };

  window._refriPracticeNext = function() {
  ...
  function _renderPracticeResults(container) {
    ...
    var passed = pct >= 70;
    var catInfo = (window.REFRI_CATEGORIAS || {})[st.categoria] || { nombre: st.categoria, color: '#666' };
    ...
    html += '<button onclick="_refriStartPractice(\'' + st.categoria + '\',' + (st.quizNum || 0) + ')" style="flex:1;padding:12px;background:linear-gradient(135deg,' + THEME_PRIMARY + ',' + THEME_DARK + ');color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">' + (typeof _t === 'function' ? _t('study_repeat', '\ud83d\udd04 Repetir') : '\ud83d\udd04 Repetir') + '</button>';
    html += '<button onclick="_refriExitPractice()" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Categorias</button>';
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._refriExitPractice = function() {
    var st = _practiceState;
  ...
  function _renderSimulacro(container) {
    ...
    html += '<h2 style="color:#0F0F0F;margin:0 0 8px;font-size:20px;">' + (typeof _t === 'function' ? _t('study_tab_mock', 'Simulacro') : 'Simulacro') + ' Refrigeraci\u00f3n</h2>';
    ...
    html += '<div style="background:#f0fdfa;border:1px solid #99f6e4;border-radius:12px;padding:14px;text-align:left;margin-bottom:16px;">';
    ...
    html += '<button onclick="_refriStartSimulacro()" style="width:100%;padding:16px;background:linear-gradient(135deg,#dc2626,#991b1b);color:#fff;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;box-shadow:0 4px 15px rgba(220,38,38,0.3);">\ud83d\ude80 ' + (typeof _t === 'function' ? _t('study_start_mock', 'Iniciar Simulacro') : 'Iniciar Simulacro') + '</button>';
  ...
  window._refriStartSimulacro = function() {
  ...
  function _updateSimulacroTimer() {
    ...
    var timerEl = document.getElementById('refriTimer');
    if (timerEl) {
  ...
  function _renderSimulacroQuestion(container) {
    ...
    html += '<div id="refriTimer" style="font-size:18px;font-weight:700;color:' + (remaining < 300 ? '#dc2626' : THEME_PRIMARY) + ';">\u23f1 ' + mins + ':' + (secs < 10 ? '0' : '') + secs + '</div>';
    html += '<div style="font-size:12px;color:#3D3D3A;">' + answered + '/100 respondidas</div>';
    html += '<button onclick="_refriFinishSimulacro()" style="background:#dc2626;color:#fff;border:none;padding:6px 12px;border-radius:6px;font-size:11px;cursor:pointer;">' + (typeof _t === 'function' ? _t('study_finish', 'Terminar') : 'Terminar') + '</button>';
    html += '</div>';
    ...
    _qf(q,'opciones').forEach(function(opt, oi) {
      ...
      html += '<button onclick="_refriSimulacroAnswer(' + oi + ')" style="' + style + '">' + letters[oi] + ') ' + _esc(opt) + '</button>';
    });

    html += '<div style="display:flex;gap:8px;margin-top:14px;">';
    if (st.currentIndex > 0) html += '<button onclick="_refriSimulacroNav(-1)" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Anterior</button>';
    if (st.currentIndex < 99) html += '<button onclick="_refriSimulacroNav(1)" style="flex:1;padding:12px;background:linear-gradient(135deg,' + THEME_PRIMARY + ',' + THEME_DARK + ');color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">Siguiente \u2192</button>';
    html += '</div>';
    ...
    for (var i = 0; i < 100; i++) {
      ...
      html += '<button onclick="_refriSimulacroGoTo(' + i + ')" style="width:24px;height:24px;border:none;border-radius:4px;font-size:9px;cursor:pointer;background:' + bg + ';color:' + clr + ';font-weight:' + (i === st.currentIndex ? '700' : '400') + ';">' + (i + 1) + '</button>';
    }
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._refriSimulacroAnswer = function(idx) {
    _simulacroState.answers[_simulacroState.currentIndex] = idx;
    _renderSimulacroQuestion(document.getElementById('refriContent'));
  };
  window._refriSimulacroNav = function(dir) {
    _simulacroState.currentIndex = Math.max(0, Math.min(99, _simulacroState.currentIndex + dir));
    _renderSimulacroQuestion(document.getElementById('refriContent'));
  };
  window._refriSimulacroGoTo = function(idx) {
    _simulacroState.currentIndex = idx;
    _renderSimulacroQuestion(document.getElementById('refriContent'));
  };

  window._refriFinishSimulacro = function() {
  ...
  function _finishSimulacro() {
    ...
    _renderSimulacroResults(document.getElementById('refriContent'));
  }
  ...
  function _renderSimulacroResults(container) {
    ...
    html += '<button onclick="_refriReviewSimulacro()" style="width:100%;margin-top:14px;padding:14px;background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">\ud83d\udcd6 ' + (typeof _t === 'function' ? _t('study_review_incorrect', 'Revisar Respuestas Incorrectas') : 'Revisar Respuestas Incorrectas') + '</button>';
    html += '<div style="display:flex;gap:8px;margin-top:8px;">';
    html += '<button onclick="_refriRetrySimulacro()" style="flex:1;padding:12px;background:linear-gradient(135deg,#dc2626,#991b1b);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\ud83d\udd04 Reintentar</button>';
    html += '<button onclick="_refriExitSimulacro()" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Inicio</button>';
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._refriReviewSimulacro = function() {
    var container = document.getElementById('refriContent');
    var st = _simulacroState;
    var letters = ['A', 'B', 'C', 'D'];
    var cats = window.REFRI_CATEGORIAS || {};
    ...
    html += '<button onclick="_refriShowResults()" style="background:#FAFAF7;border:none;color:#3D3D3A;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:12px;">\u2190 ' + (typeof _t === 'function' ? _t('study_results', 'Resultados') : 'Resultados') + '</button>';
    html += '</div>';
  ...
  window._refriShowResults = function() {
    _renderSimulacroResults(document.getElementById('refriContent'));
  };

  window._refriRetrySimulacro = function() {
    _simulacroState = { questions: [], answers: [], currentIndex: 0, startTime: null, timerInterval: null, timeLimit: 90 * 60, finished: false };
    window._refriStartSimulacro();
  };

  window._refriExitSimulacro = function() {
    _simulacroState = { questions: [], answers: [], currentIndex: 0, startTime: null, timerInterval: null, timeLimit: 90 * 60, finished: false };
    _renderTabContent();
  };
