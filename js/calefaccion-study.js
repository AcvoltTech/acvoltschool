// Calefaccion (Heating) Study System — Maestro HVACR
...
(function() {
  ...
  var QUIZ_SIZE = 38; // 150/4 per user spec
  var _practiceState = {
  ...
  function _getByCategory(cat) {
    if (!window.CALEF_QUESTIONS) return [];
    if (cat === 'ALL') return window.CALEF_QUESTIONS.slice();
    return window.CALEF_QUESTIONS.filter(function(q) { return q.categoria === cat; });
  }
  function _loadProgress(cat) {
    try { return JSON.parse(localStorage.getItem('calef_progress_' + cat)) || { correctas: 0, total: 0 }; }
    catch(e) { return { correctas: 0, total: 0 }; }
  }
  function _saveProgress(cat, correctas, total) {
    try { localStorage.setItem('calef_progress_' + cat, JSON.stringify({ correctas: correctas, total: total, fecha: new Date().toISOString() })); }
    catch(e) {}
  }
  function _loadSimulacroHistory() {
    try { return JSON.parse(localStorage.getItem('calef_simulacros')) || []; }
    catch(e) { return []; }
  }
  function _saveSimulacroResult(result) {
    try {
      ...
      localStorage.setItem('calef_simulacros', JSON.stringify(hist));
    } catch(e) {}
  }

  var CAT_KEYS = ['CALEF_ELECTRICIDAD', 'CALEF_HORNOS', 'CALEF_COMBUSTION', 'CALEF_HIDRONICA', 'CALEF_TERMODINAMICA'];
  var THEME_PRIMARY = '#E65100';
  var THEME_DARK = '#BF360C';

  // ── Main Init ──
  window.initCalefaccionStudy = function() {
    var screen = document.getElementById('calefaccionStudyScreen');
    if (!screen) return;
    _activeTab = 'estudiar';
    _renderMain(screen);
  };
  ...
  function _renderMain(screen) {
    var totalQ = window.CALEF_QUESTIONS ? window.CALEF_QUESTIONS.length : 0;
    var html = '';
    ...
    html += '<div style="flex:1;text-align:center;"><h2 style="color:#fff;margin:0;font-size:18px;">Heating Technician</h2>';
    ...
    html += '<div style="display:flex;gap:4px;background:rgba(0,0,0,0.2);border-radius:10px;padding:3px;" id="calefTabs">';
    var tabs = [
    ...
    tabs.forEach(function(tab) {
      ...
      html += '<button onclick="_calefSwitchTab(\'' + tab.id + '\')" style="flex:1;padding:8px 4px;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;' +
    ...
    html += '</div></div>';
    html += '<div id="calefContent" style="padding:12px;overflow-y:auto;flex:1;"></div>';
    screen.innerHTML = html;
    _renderTabContent();
  }

  window._calefSwitchTab = async function(tabId) {
    ...
    _activeTab = tabId;
    var tabsDiv = document.getElementById('calefTabs');
    if (tabsDiv) {
  ...
  function _renderTabContent() {
    var container = document.getElementById('calefContent');
    if (!container) return;
  ...
  function _renderEstudiar(container) {
    var cats = window.CALEF_CATEGORIAS || {};
    ...
    CAT_KEYS.forEach(function(catKey) {
      ...
      html += '<div class="calef-cat-section" id="calefCat_' + catKey + '">';
      html += '<div onclick="document.getElementById(\'calefCat_' + catKey + '\').classList.toggle(\'calef-cat-open\')" style="background:linear-gradient(135deg,' + catInfo.color + ',' + catInfo.color + 'cc);padding:12px 14px;border-radius:12px;cursor:pointer;display:flex;align-items:center;gap:10px;margin-bottom:2px;transition:all 0.2s;">';
      ...
      html += '<span style="color:#fff;font-size:18px;transition:transform 0.3s;" class="calef-cat-arrow">\u25bc</span>';
      html += '</div>';

      html += '<div class="calef-cat-body" style="display:none;padding:8px 0;">';
  ...
  function _injectStyles() {
    if (document.getElementById('calefStudyStyles')) return;
    var style = document.createElement('style');
    style.id = 'calefStudyStyles';
    style.textContent =
      '.calef-cat-section .calef-cat-body{display:none}' +
      '.calef-cat-section.calef-cat-open .calef-cat-body{display:block!important}' +
      '.calef-cat-section.calef-cat-open .calef-cat-arrow{transform:rotate(180deg)}' +
      '.calef-cat-section{margin-bottom:8px}';
    document.head.appendChild(style);
  }

  // ══════════════════════════════════════════════════
  // TAB 2 — PRACTICAR (38-question quiz chunks)
  // ══════════════════════════════════════════════════
  ...
  function _loadQuizProgress(cat, quizNum) {
    try { return JSON.parse(localStorage.getItem('calef_quiz_' + cat + '_' + quizNum)) || null; }
    catch(e) { return null; }
  }
  function _saveQuizProgress(cat, quizNum, correctas, total) {
    try { localStorage.setItem('calef_quiz_' + cat + '_' + quizNum, JSON.stringify({ correctas: correctas, total: total, fecha: new Date().toISOString() })); }
    catch(e) {}
  }
  ...
  function _renderPracticar(container) {
    ...
    CAT_KEYS.forEach(function(catKey) {
      ...
      html += '<div onclick="_calefShowQuizzes(\'' + catKey + '\')" style="background:#fff;border:2px solid ' + catInfo.color + '33;border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;transition:all 0.2s;">';
    ...
    html += '<div onclick="_calefStartPractice(\'ALL\',0)" style="background:linear-gradient(135deg,#667eea,#764ba2);border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;">';
  ...
  window._calefShowQuizzes = function(cat) {
  ...
  function _renderQuizSelector(container) {
    ...
    html += '<button onclick="_calefBackToCategories()" style="background:none;border:none;color:#3D3D3A;cursor:pointer;font-size:13px;padding:0 0 8px;">\u2190 ' + (typeof _t === 'function' ? _t('study_back_categories', 'Volver a Categorias') : 'Volver a Categorias') + '</button>';
    ...
    for (var qi = 1; qi <= numQuizzes; qi++) {
      ...
      html += '<div onclick="_calefStartPractice(\'' + cat + '\',' + qi + ')" style="background:#fff;border:2px solid ' + (completed ? (passed ? '#86efac' : '#fca5a5') : catInfo.color + '33') + ';border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;">';
  ...
  window._calefBackToCategories = function() {
    _practiceView = 'categories';
    _practiceSelectedCat = null;
    _renderTabContent();
  };

  window._calefStartPractice = function(cat, quizNum) {
    var questions = _getByCategory(cat);
  ...
  function _renderPracticeQuestion(container) {
    ...
    var catInfo = (window.CALEF_CATEGORIAS || {})[q.categoria] || { nombre: q.categoria, color: '#666' };
    var total = st.questions.length;
    var num = st.currentIndex + 1;
    var letters = ['A', 'B', 'C', 'D'];

    var html = '';
    html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;padding:8px 12px;background:#fff8f0;border-radius:10px;">';
    html += '<button onclick="_calefExitPractice()" style="background:none;border:none;color:#3D3D3A;cursor:pointer;font-size:13px;">\u2190 Salir</button>';
    ...
    _qf(q, 'opciones').forEach(function(opt, oi) {
      ...
      html += '<button onclick="_calefPracticeAnswer(' + oi + ')" style="' + btnStyle + '">';
    ...
    if (st.answered) {
      ...
      html += '</div>';
      html += '<button onclick="_calefPracticeNext()" style="display:block;width:100%;margin-top:14px;padding:14px;background:linear-gradient(135deg,' + THEME_PRIMARY + ',' + THEME_DARK + ');color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;">';
  ...
  window._calefPracticeAnswer = function(idx) {
    ...
    if (idx === q.respuesta_correcta) {
      ...
      if (window.Gamification) window.Gamification.recordAnswer('calefaccion', q.categoria || q.category || '', true);
    } else {
      _practiceState.incorrectas++;
      _practiceState.streak = 0;
      if (window.Gamification) window.Gamification.recordAnswer('calefaccion', q.categoria || q.category || '', false);
    }
    _renderPracticeQuestion(document.getElementById('calefContent'));
  };

  window._calefPracticeNext = function() {
  ...
  function _renderPracticeResults(container) {
    ...
    var passed = pct >= 70;
    var catInfo = (window.CALEF_CATEGORIAS || {})[st.categoria] || { nombre: st.categoria, color: '#666' };
    ...
    html += '<button onclick="_calefStartPractice(\'' + st.categoria + '\',' + (st.quizNum || 0) + ')" style="flex:1;padding:12px;background:linear-gradient(135deg,' + THEME_PRIMARY + ',' + THEME_DARK + ');color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">' + (typeof _t === 'function' ? _t('study_repeat', '\ud83d\udd04 Repetir') : '\ud83d\udd04 Repetir') + '</button>';
    html += '<button onclick="_calefExitPractice()" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Categorias</button>';
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._calefExitPractice = function() {
    var st = _practiceState;
  ...
  function _renderSimulacro(container) {
    ...
    html += '<h2 style="color:#0F0F0F;margin:0 0 8px;font-size:20px;">' + (typeof _t === 'function' ? _t('study_tab_mock', 'Simulacro') : 'Simulacro') + ' Calefaccion</h2>';
    ...
    html += '<div style="background:#fff8f0;border:1px solid #fed7aa;border-radius:12px;padding:14px;text-align:left;margin-bottom:16px;">';
    ...
    html += '<button onclick="_calefStartSimulacro()" style="width:100%;padding:16px;background:linear-gradient(135deg,#dc2626,#991b1b);color:#fff;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;box-shadow:0 4px 15px rgba(220,38,38,0.3);">\ud83d\ude80 ' + (typeof _t === 'function' ? _t('study_start_mock', 'Iniciar Simulacro') : 'Iniciar Simulacro') + '</button>';
  ...
  window._calefStartSimulacro = function() {
  ...
  function _updateSimulacroTimer() {
    ...
    var timerEl = document.getElementById('calefTimer');
    if (timerEl) {
  ...
  function _renderSimulacroQuestion(container) {
    ...
    html += '<div id="calefTimer" style="font-size:18px;font-weight:700;color:' + (remaining < 300 ? '#dc2626' : THEME_PRIMARY) + ';">\u23f1 ' + mins + ':' + (secs < 10 ? '0' : '') + secs + '</div>';
    html += '<div style="font-size:12px;color:#3D3D3A;">' + answered + '/100 respondidas</div>';
    html += '<button onclick="_calefFinishSimulacro()" style="background:#dc2626;color:#fff;border:none;padding:6px 12px;border-radius:6px;font-size:11px;cursor:pointer;">' + (typeof _t === 'function' ? _t('study_finish', 'Terminar') : 'Terminar') + '</button>';
    html += '</div>';
    ...
    _qf(q, 'opciones').forEach(function(opt, oi) {
      ...
      html += '<button onclick="_calefSimulacroAnswer(' + oi + ')" style="' + style + '">' + letters[oi] + ') ' + _esc(opt) + '</button>';
    });

    html += '<div style="display:flex;gap:8px;margin-top:14px;">';
    if (st.currentIndex > 0) html += '<button onclick="_calefSimulacroNav(-1)" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Anterior</button>';
    if (st.currentIndex < 99) html += '<button onclick="_calefSimulacroNav(1)" style="flex:1;padding:12px;background:linear-gradient(135deg,' + THEME_PRIMARY + ',' + THEME_DARK + ');color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">Siguiente \u2192</button>';
    html += '</div>';
    ...
    for (var i = 0; i < 100; i++) {
      ...
      html += '<button onclick="_calefSimulacroGoTo(' + i + ')" style="width:24px;height:24px;border:none;border-radius:4px;font-size:9px;cursor:pointer;background:' + bg + ';color:' + clr + ';font-weight:' + (i === st.currentIndex ? '700' : '400') + ';">' + (i + 1) + '</button>';
    }
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._calefSimulacroAnswer = function(idx) {
    _simulacroState.answers[_simulacroState.currentIndex] = idx;
    _renderSimulacroQuestion(document.getElementById('calefContent'));
  };
  window._calefSimulacroNav = function(dir) {
    _simulacroState.currentIndex = Math.max(0, Math.min(99, _simulacroState.currentIndex + dir));
    _renderSimulacroQuestion(document.getElementById('calefContent'));
  };
  window._calefSimulacroGoTo = function(idx) {
    _simulacroState.currentIndex = idx;
    _renderSimulacroQuestion(document.getElementById('calefContent'));
  };

  window._calefFinishSimulacro = function() {
  ...
  function _finishSimulacro() {
    ...
    _renderSimulacroResults(document.getElementById('calefContent'));
  }
  ...
  function _renderSimulacroResults(container) {
    ...
    html += '<button onclick="_calefReviewSimulacro()" style="width:100%;margin-top:14px;padding:14px;background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">\ud83d\udcd6 ' + (typeof _t === 'function' ? _t('study_review_incorrect', 'Revisar Respuestas Incorrectas') : 'Revisar Respuestas Incorrectas') + '</button>';
    html += '<div style="display:flex;gap:8px;margin-top:8px;">';
    html += '<button onclick="_calefRetrySimulacro()" style="flex:1;padding:12px;background:linear-gradient(135deg,#dc2626,#991b1b);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\ud83d\udd04 Reintentar</button>';
    html += '<button onclick="_calefExitSimulacro()" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Inicio</button>';
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._calefReviewSimulacro = function() {
    var container = document.getElementById('calefContent');
    var st = _simulacroState;
    var letters = ['A', 'B', 'C', 'D'];
    var cats = window.CALEF_CATEGORIAS || {};
    ...
    html += '<button onclick="_calefShowResults()" style="background:#FAFAF7;border:none;color:#3D3D3A;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:12px;">\u2190 ' + (typeof _t === 'function' ? _t('study_results', 'Resultados') : 'Resultados') + '</button>';
    html += '</div>';
  ...
  window._calefShowResults = function() {
    _renderSimulacroResults(document.getElementById('calefContent'));
  };

  window._calefRetrySimulacro = function() {
    _simulacroState = { questions: [], answers: [], currentIndex: 0, startTime: null, timerInterval: null, timeLimit: 90 * 60, finished: false };
    window._calefStartSimulacro();
  };

  window._calefExitSimulacro = function() {
    _simulacroState = { questions: [], answers: [], currentIndex: 0, startTime: null, timerInterval: null, timeLimit: 90 * 60, finished: false };
    _renderTabContent();
  };
