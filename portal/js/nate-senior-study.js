// NATE Senior Technician Study System — Maestro HVACR
...
(function() {
  ...
  function _getByCategory(cat) {
    if (!window.NATE_SR_QUESTIONS) return [];
    if (cat === 'ALL') return window.NATE_SR_QUESTIONS.slice();
    return window.NATE_SR_QUESTIONS.filter(function(q) { return q.categoria === cat; });
  }
  function _loadProgress(cat) {
    try { return JSON.parse(localStorage.getItem('ns_progress_' + cat)) || { correctas: 0, total: 0 }; }
    catch(e) { return { correctas: 0, total: 0 }; }
  }
  function _saveProgress(cat, correctas, total) {
    try { localStorage.setItem('ns_progress_' + cat, JSON.stringify({ correctas: correctas, total: total, fecha: new Date().toISOString() })); }
    catch(e) {}
  }
  function _loadSimulacroHistory() {
    try { return JSON.parse(localStorage.getItem('ns_simulacros')) || []; }
    catch(e) { return []; }
  }
  function _saveSimulacroResult(result) {
    try {
      ...
      localStorage.setItem('ns_simulacros', JSON.stringify(hist));
    } catch(e) {}
  }

  // ── Main Init ──
  window.initNateSeniorStudy = function() {
    var screen = document.getElementById('nateSeniorStudyScreen');
    if (!screen) return;
    _activeTab = 'estudiar';
    _renderMain(screen);
  };
  ...
  function _renderMain(screen) {
    var html = '';
    html += '<div style="background:linear-gradient(135deg,#1B5E20,#2E7D32);padding:16px 12px 12px;border-radius:0 0 20px 20px;margin:-1px -1px 0;">';
    ...
    html += '<div style="flex:1;text-align:center;"><h2 style="color:#fff;margin:0;font-size:18px;">NATE Senior \u2014 Technician</h2>';
    html += '<p style="color:rgba(255,255,255,0.7);margin:2px 0 0;font-size:12px;">' + 1000 + ' ' + (typeof _t === 'function' ? _t('study_prep_q', 'preguntas de preparacion') : 'preguntas de preparacion') + '</p></div>';
    html += '<div style="width:60px;"></div></div>';

    html += '<div style="display:flex;gap:4px;background:rgba(0,0,0,0.2);border-radius:10px;padding:3px;" id="nsTabs">';
    var tabs = [
    ...
    tabs.forEach(function(tab) {
      ...
      html += '<button onclick="_nsSwitchTab(\'' + tab.id + '\')" style="flex:1;padding:8px 4px;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;' +
        (isActive ? 'background:#fff;color:#1B5E20;box-shadow:0 2px 8px rgba(0,0,0,0.15);' : 'background:transparent;color:rgba(255,255,255,0.7);') +
        '">' + tab.icon + ' ' + tab.label + '</button>';
    });
    html += '</div></div>';
    html += '<div id="nsContent" style="padding:12px;overflow-y:auto;flex:1;"></div>';
    screen.innerHTML = html;
    _renderTabContent();
  }

  window._nsSwitchTab = async function(tabId) {
    ...
    _activeTab = tabId;
    var tabsDiv = document.getElementById('nsTabs');
    if (tabsDiv) {
      var btns = tabsDiv.querySelectorAll('button');
      var tabIds = ['estudiar', 'practicar', 'simulacro'];
      btns.forEach(function(btn, i) {
        if (tabIds[i] === tabId) {
          btn.style.background = '#fff'; btn.style.color = '#1B5E20'; btn.style.boxShadow = '0 2px 8px rgba(0,0,0,0.15)';
        } else {
  ...
  function _renderTabContent() {
    var container = document.getElementById('nsContent');
    if (!container) return;
  ...
  function _renderEstudiar(container) {
    var cats = window.NATE_SR_CATEGORIAS || {};
    ...
    var catKeys = ['NATE_SR_CHILLERS', 'NATE_SR_RACK', 'NATE_SR_HVAC', 'NATE_SR_MINISPLIT', 'NATE_SR_HEATPUMP', 'NATE_SR_PACKAGE', 'NATE_SR_MANAGEMENT', 'NATE_SR_CEO'];
    var letters = ['A', 'B', 'C', 'D'];
    ...
    catKeys.forEach(function(catKey) {
      ...
      html += '<div class="ns-cat-section" id="nsCat_' + catKey + '">';
      html += '<div onclick="document.getElementById(\'nsCat_' + catKey + '\').classList.toggle(\'ns-cat-open\')" style="background:linear-gradient(135deg,' + catInfo.color + ',' + catInfo.color + 'cc);padding:12px 14px;border-radius:12px;cursor:pointer;display:flex;align-items:center;gap:10px;margin-bottom:2px;transition:all 0.2s;">';
      ...
      html += '<span style="color:#fff;font-size:18px;transition:transform 0.3s;" class="ns-cat-arrow">\u25bc</span>';
      html += '</div>';

      html += '<div class="ns-cat-body" style="display:none;padding:8px 0;">';
      questions.forEach(function(q, qi) {
        html += '<div style="background:#FAFAF7;border:1px solid #E7E5DE;border-radius:10px;padding:12px;margin-bottom:8px;">';
        html += '<div style="font-weight:600;color:#0F0F0F;font-size:13px;margin-bottom:8px;">' + (qi + 1) + '. ' + _esc(q.pregunta) + '</div>';
        q.opciones.forEach(function(opt, oi) {
        ...
        if (q.explicacion) {
          html += '<div style="margin-top:8px;padding:8px 10px;background:#eff6ff;border-left:3px solid #3b82f6;border-radius:0 6px 6px 0;font-size:11px;color:#1e40af;font-style:italic;">\ud83d\udca1 ' + _esc(q.explicacion) + '</div>';
        }
        html += '</div>';
      });
      html += '</div></div>';
  ...
  function _injectStyles() {
    if (document.getElementById('nsStudyStyles')) return;
    var style = document.createElement('style');
    style.id = 'nsStudyStyles';
    style.textContent =
      '.ns-cat-section .ns-cat-body{display:none}' +
      '.ns-cat-section.ns-cat-open .ns-cat-body{display:block!important}' +
      '.ns-cat-section.ns-cat-open .ns-cat-arrow{transform:rotate(180deg)}' +
      '.ns-cat-section{margin-bottom:8px}';
  ...
  function _loadQuizProgress(cat, quizNum) {
    try { return JSON.parse(localStorage.getItem('ns_quiz_' + cat + '_' + quizNum)) || null; }
    catch(e) { return null; }
  }
  function _saveQuizProgress(cat, quizNum, correctas, total) {
    try { localStorage.setItem('ns_quiz_' + cat + '_' + quizNum, JSON.stringify({ correctas: correctas, total: total, fecha: new Date().toISOString() })); }
    catch(e) {}
  }
  ...
  function _renderPracticar(container) {
    ...
    html += '<p style="color:#3D3D3A;font-size:13px;margin:0;">' + (typeof _t === 'function' ? _t('study_choose_cat', 'Elige una categoria. Quizzes de 25 preguntas cada uno.') : 'Elige una categoria. Quizzes de 25 preguntas cada uno.') + '</p></div>';
    ...
    catKeys.forEach(function(catKey) {
      ...
      html += '<div onclick="_nsShowQuizzes(\'' + catKey + '\')" style="background:#fff;border:2px solid ' + catInfo.color + '33;border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;transition:all 0.2s;">';
    ...
    html += '<div onclick="_nsStartPractice(\'ALL\',0)" style="background:linear-gradient(135deg,#667eea,#764ba2);border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;">';
  ...
  window._nsShowQuizzes = function(cat) {
  ...
  function _renderQuizSelector(container) {
    ...
    html += '<button onclick="_nsBackToCategories()" style="background:none;border:none;color:#3D3D3A;cursor:pointer;font-size:13px;padding:0 0 8px;">\u2190 ' + (typeof _t === 'function' ? _t('study_back_categories', 'Volver a Categorias') : 'Volver a Categorias') + '</button>';
    ...
    for (var qi = 1; qi <= numQuizzes; qi++) {
      ...
      html += '<div onclick="_nsStartPractice(\'' + cat + '\',' + qi + ')" style="background:#fff;border:2px solid ' + (completed ? (passed ? '#86efac' : '#fca5a5') : catInfo.color + '33') + ';border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;">';
  ...
  window._nsBackToCategories = function() {
    _practiceView = 'categories';
    _practiceSelectedCat = null;
    _renderTabContent();
  };

  window._nsStartPractice = function(cat, quizNum) {
    var questions = _getByCategory(cat);
  ...
  function _renderPracticeQuestion(container) {
    ...
    var catInfo = (window.NATE_SR_CATEGORIAS || {})[q.categoria] || { nombre: q.categoria, color: '#666' };
    ...
    html += '<button onclick="_nsExitPractice()" style="background:none;border:none;color:#3D3D3A;cursor:pointer;font-size:13px;">\u2190 Salir</button>';
    html += '<div style="font-size:12px;color:#1B5E20;font-weight:600;">' + num + ' / ' + total + '</div>';
    ...
    html += '<div style="height:4px;background:#E7E5DE;border-radius:2px;margin-bottom:14px;overflow:hidden;"><div style="height:100%;width:' + ((num / total) * 100) + '%;background:linear-gradient(90deg,#2E7D32,#1B5E20);border-radius:2px;transition:width 0.3s;"></div></div>';
    ...
    html += '<div style="font-size:15px;font-weight:600;color:#0F0F0F;margin-bottom:14px;line-height:1.5;">' + _esc(q.pregunta) + '</div>';
    ...
    q.opciones.forEach(function(opt, oi) {
      ...
      html += '<button onclick="_nsPracticeAnswer(' + oi + ')" style="' + btnStyle + '">';
    ...
    if (st.answered) {
      ...
      if (q.explicacion) html += '<div style="font-size:12px;color:#3D3D3A;line-height:1.5;">\ud83d\udca1 ' + _esc(q.explicacion) + '</div>';
      html += '</div>';
      html += '<button onclick="_nsPracticeNext()" style="display:block;width:100%;margin-top:14px;padding:14px;background:linear-gradient(135deg,#1B5E20,#2E7D32);color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;">';
  ...
  window._nsPracticeAnswer = function(idx) {
    ...
    if (idx === q.respuesta_correcta) {
      ...
      if (window.Gamification) window.Gamification.recordAnswer('nate_senior', q.categoria || q.category || '', true);
    } else {
      _practiceState.incorrectas++;
      _practiceState.streak = 0;
      if (window.Gamification) window.Gamification.recordAnswer('nate_senior', q.categoria || q.category || '', false);
    }
    _renderPracticeQuestion(document.getElementById('nsContent'));
  };

  window._nsPracticeNext = function() {
  ...
  function _renderPracticeResults(container) {
    ...
    var passed = pct >= 70;
    var catInfo = (window.NATE_SR_CATEGORIAS || {})[st.categoria] || { nombre: st.categoria, color: '#666' };
    ...
    html += '<button onclick="_nsStartPractice(\'' + st.categoria + '\',' + (st.quizNum || 0) + ')" style="flex:1;padding:12px;background:linear-gradient(135deg,#1B5E20,#2E7D32);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">' + (typeof _t === 'function' ? _t('study_repeat', '\ud83d\udd04 Repetir') : '\ud83d\udd04 Repetir') + '</button>';
    html += '<button onclick="_nsExitPractice()" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Categorias</button>';
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._nsExitPractice = function() {
    var st = _practiceState;
  ...
  function _renderSimulacro(container) {
    ...
    html += '<h2 style="color:#0F0F0F;margin:0 0 8px;font-size:20px;">' + (typeof _t === 'function' ? _t('study_tab_mock', 'Simulacro') : 'Simulacro') + ' NATE Senior</h2>';
    ...
    html += '<div style="font-weight:700;color:#1B5E20;margin-bottom:8px;">\ud83d\udccb ' + (typeof _t === 'function' ? _t('study_mock_rules', 'Reglas del Simulacro') : 'Reglas del Simulacro') + '</div>';
    html += '<ul style="color:#3D3D3A;font-size:12px;margin:0;padding-left:20px;line-height:1.8;">';
    html += '<li>' + (typeof _t === 'function' ? _t('study_mock_rule_q_mixed', '100 preguntas mezcladas de las 8 secciones') : '100 preguntas mezcladas de las 8 secciones') + '</li>';
    ...
    html += '<button onclick="_nsStartSimulacro()" style="width:100%;padding:16px;background:linear-gradient(135deg,#1B5E20,#2E7D32);color:#fff;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;box-shadow:0 4px 15px rgba(27,94,32,0.3);">\ud83d\ude80 ' + (typeof _t === 'function' ? _t('study_start_mock', 'Iniciar Simulacro') : 'Iniciar Simulacro') + '</button>';
  ...
  window._nsStartSimulacro = function() {
  ...
  function _updateSimulacroTimer() {
    ...
    var timerEl = document.getElementById('nsSimTimer');
    if (timerEl) {
      var mins = Math.floor(remaining / 60);
      var secs = remaining % 60;
      timerEl.textContent = '\u23f1 ' + mins + ':' + (secs < 10 ? '0' : '') + secs;
      timerEl.style.color = remaining < 300 ? '#dc2626' : '#1B5E20';
    }
  }
  ...
  function _renderSimulacroQuestion(container) {
    ...
    html += '<div id="nsSimTimer" style="font-size:18px;font-weight:700;color:' + (remaining < 300 ? '#dc2626' : '#1B5E20') + ';">\u23f1 ' + mins + ':' + (secs < 10 ? '0' : '') + secs + '</div>';
    html += '<div style="font-size:12px;color:#3D3D3A;">' + answered + '/100 respondidas</div>';
    html += '<button onclick="_nsFinishSimulacro()" style="background:#dc2626;color:#fff;border:none;padding:6px 12px;border-radius:6px;font-size:11px;cursor:pointer;">' + (typeof _t === 'function' ? _t('study_finish', 'Terminar') : 'Terminar') + '</button>';
    html += '</div>';

    html += '<div style="height:4px;background:#E7E5DE;border-radius:2px;margin-bottom:10px;overflow:hidden;"><div style="height:100%;width:' + (num / 100 * 100) + '%;background:linear-gradient(90deg,#1B5E20,#2E7D32);border-radius:2px;transition:width 0.3s;"></div></div>';
    ...
    html += '</div>';
    html += '<div style="font-size:14px;font-weight:600;color:#0F0F0F;margin-bottom:12px;line-height:1.5;">' + _esc(q.pregunta) + '</div>';
    ...
    q.opciones.forEach(function(opt, oi) {
      ...
      html += '<button onclick="_nsSimulacroAnswer(' + oi + ')" style="' + style + '">' + letters[oi] + ') ' + _esc(opt) + '</button>';
    });

    html += '<div style="display:flex;gap:8px;margin-top:14px;">';
    if (st.currentIndex > 0) html += '<button onclick="_nsSimulacroNav(-1)" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Anterior</button>';
    if (st.currentIndex < 99) html += '<button onclick="_nsSimulacroNav(1)" style="flex:1;padding:12px;background:linear-gradient(135deg,#1B5E20,#2E7D32);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">Siguiente \u2192</button>';
    html += '</div>';
    ...
    for (var i = 0; i < 100; i++) {
      ...
      if (st.answers[i] >= 0) { bg = '#dbeafe'; clr = '#1B5E20'; }
      if (i === st.currentIndex) { bg = '#1B5E20'; clr = '#fff'; }
      html += '<button onclick="_nsSimulacroGoTo(' + i + ')" style="width:24px;height:24px;border:none;border-radius:4px;font-size:9px;cursor:pointer;background:' + bg + ';color:' + clr + ';font-weight:' + (i === st.currentIndex ? '700' : '400') + ';">' + (i + 1) + '</button>';
    }
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._nsSimulacroAnswer = function(idx) {
    _simulacroState.answers[_simulacroState.currentIndex] = idx;
    _renderSimulacroQuestion(document.getElementById('nsContent'));
  };
  window._nsSimulacroNav = function(dir) {
    _simulacroState.currentIndex = Math.max(0, Math.min(99, _simulacroState.currentIndex + dir));
    _renderSimulacroQuestion(document.getElementById('nsContent'));
  };
  window._nsSimulacroGoTo = function(idx) {
    _simulacroState.currentIndex = idx;
    _renderSimulacroQuestion(document.getElementById('nsContent'));
  };

  window._nsFinishSimulacro = function() {
  ...
  function _finishSimulacro() {
    ...
    _renderSimulacroResults(document.getElementById('nsContent'));
  }
  ...
  function _renderSimulacroResults(container) {
    ...
    html += '<button onclick="_nsReviewSimulacro()" style="width:100%;margin-top:14px;padding:14px;background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">\ud83d\udcd6 ' + (typeof _t === 'function' ? _t('study_review_incorrect', 'Revisar Respuestas Incorrectas') : 'Revisar Respuestas Incorrectas') + '</button>';
    html += '<div style="display:flex;gap:8px;margin-top:8px;">';
    html += '<button onclick="_nsRetrySimulacro()" style="flex:1;padding:12px;background:linear-gradient(135deg,#1B5E20,#2E7D32);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\ud83d\udd04 Reintentar</button>';
    html += '<button onclick="_nsExitSimulacro()" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Inicio</button>';
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._nsReviewSimulacro = function() {
    var container = document.getElementById('nsContent');
    var st = _simulacroState;
    var letters = ['A', 'B', 'C', 'D'];
    var cats = window.NATE_SR_CATEGORIAS || {};
    ...
    html += '<button onclick="_nsShowResults()" style="background:#FAFAF7;border:none;color:#3D3D3A;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:12px;">\u2190 ' + (typeof _t === 'function' ? _t('study_results', 'Resultados') : 'Resultados') + '</button>';
    html += '</div>';
    ...
    st.questions.forEach(function(q, i) {
      ...
      html += '<div style="font-weight:600;color:#0F0F0F;font-size:12px;margin-bottom:6px;">' + _esc(q.pregunta) + '</div>';
      if (userAns >= 0) html += '<div style="padding:4px 8px;background:#fef2f2;border-radius:4px;font-size:11px;color:#991b1b;margin-bottom:3px;">' + (typeof _t === 'function' ? _t('study_your_answer', 'Tu respuesta') : 'Tu respuesta') + ': ' + letters[userAns] + ') ' + _esc(q.opciones[userAns]) + ' \u2717</div>';
      ...
      html += '<div style="padding:4px 8px;background:#dcfce7;border-radius:4px;font-size:11px;color:#166534;">' + (typeof _t === 'function' ? _t('study_correct', 'Correcta') : 'Correcta') + ': ' + letters[q.respuesta_correcta] + ') ' + _esc(q.opciones[q.respuesta_correcta]) + ' \u2713</div>';
      if (q.explicacion) html += '<div style="margin-top:4px;font-size:10px;color:#1e40af;font-style:italic;">\ud83d\udca1 ' + _esc(q.explicacion) + '</div>';
      html += '</div>';
    });
  ...
  window._nsShowResults = function() {
    _renderSimulacroResults(document.getElementById('nsContent'));
  };

  window._nsRetrySimulacro = function() {
    _simulacroState = { questions: [], answers: [], currentIndex: 0, startTime: null, timerInterval: null, timeLimit: 90 * 60, finished: false };
    window._nsStartSimulacro();
  };

  window._nsExitSimulacro = function() {
    _simulacroState = { questions: [], answers: [], currentIndex: 0, startTime: null, timerInterval: null, timeLimit: 90 * 60, finished: false };
    _renderTabContent();
  };
