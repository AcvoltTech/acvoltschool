// ET Card Study System — Maestro HVACR
...
(function() {
  ...
  function _getByCategory(cat) {
    if (!window.ET_QUESTIONS) return [];
    if (cat === 'ALL') return window.ET_QUESTIONS.slice();
    return window.ET_QUESTIONS.filter(function(q) { return q.categoria === cat; });
  }
  function _loadProgress(cat) {
    try { return JSON.parse(localStorage.getItem('et_progress_' + cat)) || { correctas: 0, total: 0 }; }
    catch(e) { return { correctas: 0, total: 0 }; }
  }
  function _saveProgress(cat, correctas, total) {
    try { localStorage.setItem('et_progress_' + cat, JSON.stringify({ correctas: correctas, total: total, fecha: new Date().toISOString() })); }
    catch(e) {}
  }
  function _loadSimulacroHistory() {
    try { return JSON.parse(localStorage.getItem('et_simulacros')) || []; }
    catch(e) { return []; }
  }
  function _saveSimulacroResult(result) {
    try {
      ...
      localStorage.setItem('et_simulacros', JSON.stringify(hist));
    } catch(e) {}
  }

  // ── Main Init ──
  window.initEtStudy = function() {
    var screen = document.getElementById('etStudyScreen');
    if (!screen) return;
    _activeTab = 'estudiar';
    _renderMain(screen);
  };
  ...
  function _renderMain(screen) {
    var html = '';
    html += '<div style="background:linear-gradient(135deg,#F57F17,#E65100);padding:16px 12px 12px;border-radius:0 0 20px 20px;margin:-1px -1px 0;">';
    ...
    html += '<div style="flex:1;text-align:center;"><h2 style="color:#fff;margin:0;font-size:18px;">ET Card \u2014 Electrician Trainee</h2>';
    html += '<p style="color:rgba(255,255,255,0.7);margin:2px 0 0;font-size:12px;">' + 694 + ' ' + (typeof _t === 'function' ? _t('study_prep_q', 'preguntas de preparacion') : 'preguntas de preparacion') + '</p></div>';
    html += '<div style="width:60px;"></div></div>';

    html += '<div style="display:flex;gap:4px;background:rgba(0,0,0,0.2);border-radius:10px;padding:3px;" id="etTabs">';
    var tabs = [
    ...
    tabs.forEach(function(tab) {
      ...
      html += '<button onclick="_etSwitchTab(\'' + tab.id + '\')" style="flex:1;padding:8px 4px;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;transition:all 0.2s;' +
        (isActive ? 'background:#fff;color:#F57F17;box-shadow:0 2px 8px rgba(0,0,0,0.15);' : 'background:transparent;color:rgba(255,255,255,0.7);') +
        '">' + tab.icon + ' ' + tab.label + '</button>';
    });
    html += '</div></div>';
    html += '<div id="etContent" style="padding:12px;overflow-y:auto;flex:1;"></div>';
    screen.innerHTML = html;
    _renderTabContent();
  }

  window._etSwitchTab = async function(tabId) {
    ...
    _activeTab = tabId;
    var tabsDiv = document.getElementById('etTabs');
    if (tabsDiv) {
      var btns = tabsDiv.querySelectorAll('button');
      var tabIds = ['estudiar', 'practicar', 'simulacro'];
      btns.forEach(function(btn, i) {
        if (tabIds[i] === tabId) {
          btn.style.background = '#fff'; btn.style.color = '#F57F17'; btn.style.boxShadow = '0 2px 8px rgba(0,0,0,0.15)';
        } else {
  ...
  function _renderEstudiar(container) {
    ...
    var catKeys = ['ET_OHM_WATT', 'ET_NEC', 'ET_CONDUCTORES', 'ET_CANALIZACIONES', 'ET_CIRCUITOS', 'ET_SERVICIO', 'ET_OVERCURRENT', 'ET_MOTORES', 'ET_GROUNDING', 'ET_TRANSFORMADORES', 'ET_ILUMINACION', 'ET_HAZARDOUS', 'ET_BAJA_TENSION', 'ET_SEGURIDAD'];
    var letters = ['A', 'B', 'C', 'D'];
    ...
    catKeys.forEach(function(catKey) {
      ...
      html += '<div class="et-cat-section" id="etCat_' + catKey + '">';
      html += '<div onclick="document.getElementById(\'etCat_' + catKey + '\').classList.toggle(\'et-cat-open\')" style="background:linear-gradient(135deg,' + catInfo.color + ',' + catInfo.color + 'cc);padding:12px 14px;border-radius:12px;cursor:pointer;display:flex;align-items:center;gap:10px;margin-bottom:2px;transition:all 0.2s;">';
      ...
      html += '<span style="color:#fff;font-size:18px;transition:transform 0.3s;" class="et-cat-arrow">\u25bc</span>';
      html += '</div>';

      html += '<div class="et-cat-body" style="display:none;padding:8px 0;">';
  ...
  function _injectStyles() {
    if (document.getElementById('etStudyStyles')) return;
    var style = document.createElement('style');
    style.id = 'etStudyStyles';
    style.textContent =
      '.et-cat-section .et-cat-body{display:none}' +
      '.et-cat-section.et-cat-open .et-cat-body{display:block!important}' +
      '.et-cat-section.et-cat-open .et-cat-arrow{transform:rotate(180deg)}' +
      '.et-cat-section{margin-bottom:8px}';
  ...
  function _loadQuizProgress(cat, quizNum) {
    try { return JSON.parse(localStorage.getItem('et_quiz_' + cat + '_' + quizNum)) || null; }
    catch(e) { return null; }
  }
  function _saveQuizProgress(cat, quizNum, correctas, total) {
    try { localStorage.setItem('et_quiz_' + cat + '_' + quizNum, JSON.stringify({ correctas: correctas, total: total, fecha: new Date().toISOString() })); }
    catch(e) {}
  }
  ...
  function _renderPracticar(container) {
    ...
    catKeys.forEach(function(catKey) {
      ...
      html += '<div onclick="_etShowQuizzes(\'' + catKey + '\')" style="background:#fff;border:2px solid ' + catInfo.color + '33;border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;transition:all 0.2s;">';
    ...
    html += '<div onclick="_etStartPractice(\'ALL\',0)" style="background:linear-gradient(135deg,#667eea,#764ba2);border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;">';
  ...
  window._etShowQuizzes = function(cat) {
  ...
  function _renderQuizSelector(container) {
    ...
    html += '<button onclick="_etBackToCategories()" style="background:none;border:none;color:#3D3D3A;cursor:pointer;font-size:13px;padding:0 0 8px;">\u2190 ' + (typeof _t === 'function' ? _t('study_back_categories', 'Volver a Categorias') : 'Volver a Categorias') + '</button>';
    ...
    for (var qi = 1; qi <= numQuizzes; qi++) {
      ...
      html += '<div onclick="_etStartPractice(\'' + cat + '\',' + qi + ')" style="background:#fff;border:2px solid ' + (completed ? (passed ? '#86efac' : '#fca5a5') : catInfo.color + '33') + ';border-radius:12px;padding:14px;margin-bottom:8px;cursor:pointer;display:flex;align-items:center;gap:12px;">';
  ...
  window._etBackToCategories = function() {
    _practiceView = 'categories';
    _practiceSelectedCat = null;
    _renderTabContent();
  };

  window._etStartPractice = function(cat, quizNum) {
    var questions = _getByCategory(cat);
  ...
  function _renderPracticeQuestion(container) {
    ...
    var catInfo = (window.ET_CATEGORIAS || {})[q.categoria] || { nombre: q.categoria, color: '#666' };
    ...
    html += '<button onclick="_etExitPractice()" style="background:none;border:none;color:#3D3D3A;cursor:pointer;font-size:13px;">\u2190 Salir</button>';
    html += '<div style="font-size:12px;color:#F57F17;font-weight:600;">' + num + ' / ' + total + '</div>';
    ...
    q.opciones.forEach(function(opt, oi) {
      ...
      html += '<button onclick="_etPracticeAnswer(' + oi + ')" style="' + btnStyle + '">';
    ...
    if (st.answered) {
      ...
      html += '</div>';
      html += '<button onclick="_etPracticeNext()" style="display:block;width:100%;margin-top:14px;padding:14px;background:linear-gradient(135deg,#F57F17,#E65100);color:#fff;border:none;border-radius:12px;font-size:15px;font-weight:700;cursor:pointer;">';
  ...
  window._etPracticeAnswer = function(idx) {
    ...
    if (idx === q.respuesta_correcta) {
      ...
      if (window.Gamification) window.Gamification.recordAnswer('et', q.categoria || q.category || '', true);
    } else {
      _practiceState.incorrectas++;
      _practiceState.streak = 0;
      if (window.Gamification) window.Gamification.recordAnswer('et', q.categoria || q.category || '', false);
    }
    _renderPracticeQuestion(document.getElementById('etContent'));
  };

  window._etPracticeNext = function() {
  ...
  function _renderPracticeResults(container) {
    ...
    var passed = pct >= 70;
    var catInfo = (window.ET_CATEGORIAS || {})[st.categoria] || { nombre: st.categoria, color: '#666' };
    ...
    html += '<button onclick="_etStartPractice(\'' + st.categoria + '\',' + (st.quizNum || 0) + ')" style="flex:1;padding:12px;background:linear-gradient(135deg,#F57F17,#E65100);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">' + (typeof _t === 'function' ? _t('study_repeat', '\ud83d\udd04 Repetir') : '\ud83d\udd04 Repetir') + '</button>';
    html += '<button onclick="_etExitPractice()" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Categorias</button>';
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._etExitPractice = function() {
    var st = _practiceState;
  ...
  function _renderSimulacro(container) {
    ...
    html += '<h2 style="color:#0F0F0F;margin:0 0 8px;font-size:20px;">' + (typeof _t === 'function' ? _t('study_tab_mock', 'Simulacro') : 'Simulacro') + ' ET Card</h2>';
    ...
    html += '<div style="font-weight:700;color:#F57F17;margin-bottom:8px;">\ud83d\udccb ' + (typeof _t === 'function' ? _t('study_mock_rules', 'Reglas del Simulacro') : 'Reglas del Simulacro') + '</div>';
    html += '<ul style="color:#3D3D3A;font-size:12px;margin:0;padding-left:20px;line-height:1.8;">';
    html += '<li>' + (typeof _t === 'function' ? _t('study_mock_rule_q_mixed', '100 preguntas mezcladas de las 14 secciones') : '100 preguntas mezcladas de las 14 secciones') + '</li>';
    ...
    html += '<button onclick="_etStartSimulacro()" style="width:100%;padding:16px;background:linear-gradient(135deg,#E65100,#BF360C);color:#fff;border:none;border-radius:12px;font-size:16px;font-weight:700;cursor:pointer;box-shadow:0 4px 15px rgba(230,81,0,0.3);">\ud83d\ude80 ' + (typeof _t === 'function' ? _t('study_start_mock', 'Iniciar Simulacro') : 'Iniciar Simulacro') + '</button>';
  ...
  window._etStartSimulacro = function() {
  ...
  function _updateSimulacroTimer() {
    ...
    var timerEl = document.getElementById('etSimTimer');
    if (timerEl) {
      var mins = Math.floor(remaining / 60);
      var secs = remaining % 60;
      timerEl.textContent = '\u23f1 ' + mins + ':' + (secs < 10 ? '0' : '') + secs;
      timerEl.style.color = remaining < 300 ? '#dc2626' : '#F57F17';
    }
  }
  ...
  function _renderSimulacroQuestion(container) {
    ...
    html += '<div id="etSimTimer" style="font-size:18px;font-weight:700;color:' + (remaining < 300 ? '#dc2626' : '#F57F17') + ';">\u23f1 ' + mins + ':' + (secs < 10 ? '0' : '') + secs + '</div>';
    html += '<div style="font-size:12px;color:#3D3D3A;">' + answered + '/100 respondidas</div>';
    html += '<button onclick="_etFinishSimulacro()" style="background:#dc2626;color:#fff;border:none;padding:6px 12px;border-radius:6px;font-size:11px;cursor:pointer;">' + (typeof _t === 'function' ? _t('study_finish', 'Terminar') : 'Terminar') + '</button>';
    html += '</div>';
    ...
    q.opciones.forEach(function(opt, oi) {
      ...
      html += '<button onclick="_etSimulacroAnswer(' + oi + ')" style="' + style + '">' + letters[oi] + ') ' + _esc(opt) + '</button>';
    });

    html += '<div style="display:flex;gap:8px;margin-top:14px;">';
    if (st.currentIndex > 0) html += '<button onclick="_etSimulacroNav(-1)" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Anterior</button>';
    if (st.currentIndex < 99) html += '<button onclick="_etSimulacroNav(1)" style="flex:1;padding:12px;background:linear-gradient(135deg,#F57F17,#E65100);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">Siguiente \u2192</button>';
    html += '</div>';
    ...
    for (var i = 0; i < 100; i++) {
      ...
      if (st.answers[i] >= 0) { bg = '#dbeafe'; clr = '#F57F17'; }
      if (i === st.currentIndex) { bg = '#F57F17'; clr = '#fff'; }
      html += '<button onclick="_etSimulacroGoTo(' + i + ')" style="width:24px;height:24px;border:none;border-radius:4px;font-size:9px;cursor:pointer;background:' + bg + ';color:' + clr + ';font-weight:' + (i === st.currentIndex ? '700' : '400') + ';">' + (i + 1) + '</button>';
    }
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._etSimulacroAnswer = function(idx) {
    _simulacroState.answers[_simulacroState.currentIndex] = idx;
    _renderSimulacroQuestion(document.getElementById('etContent'));
  };
  window._etSimulacroNav = function(dir) {
    _simulacroState.currentIndex = Math.max(0, Math.min(99, _simulacroState.currentIndex + dir));
    _renderSimulacroQuestion(document.getElementById('etContent'));
  };
  window._etSimulacroGoTo = function(idx) {
    _simulacroState.currentIndex = idx;
    _renderSimulacroQuestion(document.getElementById('etContent'));
  };

  window._etFinishSimulacro = function() {
  ...
  function _finishSimulacro() {
    ...
    _renderSimulacroResults(document.getElementById('etContent'));
  }
  ...
  function _renderSimulacroResults(container) {
    ...
    html += '<button onclick="_etReviewSimulacro()" style="width:100%;margin-top:14px;padding:14px;background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">\ud83d\udcd6 ' + (typeof _t === 'function' ? _t('study_review_incorrect', 'Revisar Respuestas Incorrectas') : 'Revisar Respuestas Incorrectas') + '</button>';
    html += '<div style="display:flex;gap:8px;margin-top:8px;">';
    html += '<button onclick="_etRetrySimulacro()" style="flex:1;padding:12px;background:linear-gradient(135deg,#E65100,#BF360C);color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\ud83d\udd04 Reintentar</button>';
    html += '<button onclick="_etExitSimulacro()" style="flex:1;padding:12px;background:#FAFAF7;color:#3D3D3A;border:none;border-radius:10px;font-size:14px;font-weight:600;cursor:pointer;">\u2190 Inicio</button>';
    html += '</div></div>';
    container.innerHTML = html;
  }

  window._etReviewSimulacro = function() {
    var container = document.getElementById('etContent');
    var st = _simulacroState;
    var letters = ['A', 'B', 'C', 'D'];
    var cats = window.ET_CATEGORIAS || {};
    ...
    html += '<button onclick="_etShowResults()" style="background:#FAFAF7;border:none;color:#3D3D3A;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:12px;">\u2190 ' + (typeof _t === 'function' ? _t('study_results', 'Resultados') : 'Resultados') + '</button>';
    html += '</div>';
  ...
  window._etShowResults = function() {
    _renderSimulacroResults(document.getElementById('etContent'));
  };

  window._etRetrySimulacro = function() {
    _simulacroState = { questions: [], answers: [], currentIndex: 0, startTime: null, timerInterval: null, timeLimit: 90 * 60, finished: false };
    window._etStartSimulacro();
  };

  window._etExitSimulacro = function() {
    _simulacroState = { questions: [], answers: [], currentIndex: 0, startTime: null, timerInterval: null, timeLimit: 90 * 60, finished: false };
    _renderTabContent();
  };
